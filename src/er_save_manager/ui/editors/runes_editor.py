...
class RunesEditor:
	...
	def setup_ui(self):
		...
		ctk.CTkLabel(row_wallet, text="Runes carried", width=170, anchor="w").pack(side=ctk.LEFT, padx=(0, 8))
		ctk.CTkEntry(row_wallet, textvariable=self.runes_var, width=220).pack(side=ctk.LEFT)

		row_mem = ctk.CTkFrame(frame, fg_color="transparent")
		row_mem.pack(fill=ctk.X, padx=8, pady=4)

		ctk.CTkLabel(row_mem, text="Dropped / lost pile", width=170, anchor="w").pack(side=ctk.LEFT, padx=(0, 8))
		ctk.CTkEntry(row_mem, textvariable=self.runes_memory_var, width=220).pack(side=ctk.LEFT)
	...
	def apply_changes(self):
		...
		if not save_file:
			CTkMessageBox.showwarning("No Save", "Load a save first.", parent=self.parent)
			return

		slot_idx = self.get_char_slot()
		if slot_idx < 0 or slot_idx > 9:
			CTkMessageBox.showwarning("Slot", "Choose Load character above first.", parent=self.parent)
			return

		slot = save_file.characters[slot_idx]
		if slot.is_empty():
			CTkMessageBox.showwarning("Empty", f"Slot {slot_idx + 1} has no character.", parent=self.parent)
			return
		...
		char.runes_memory = _clamp_u32(int_from_tk_str_var(self.runes_memory_var, char.runes_memory))

		try:
			rebuilt = rebuild_slot(slot)
			if len(rebuilt) != SLOT_DATA_SIZE:
				raise ValueError(f"Rebuilt slot size mismatch: {len(rebuilt)} != {SLOT_DATA_SIZE}")
			...
			try:
				slot_buf = BytesIO(bytes(save_file._raw_data[abs_offset : abs_offset + len(rebuilt)]))
